function [inputs, labels, set] = patches_generation_for_diff_net(size_input,size_label,stride,folder,mode,max_numPatches,batchSize)

inputs  = zeros(size_input, size_input, 4, 1,'single');
labels  = zeros(size_label, size_label, 4, 1,'single');
count   = 0;
...
%     end
       ...
       % augment data
        im_label  = im2single(image(:,:,5:8)); % single
        im_input=im2single(image(:,:,1:4));
...
labels = inputs-shave(labels,[padding,padding]); %%% residual image patches; pay attention to this!!!
