...
if __name__ == '__main__':
    ...
    for i in range(args.iteration):  # 迭代次数
        print(i)
        y_ = x1_ / np.max(x1_)
        ...
        x1 = x1 * np.max(y)
        x1_ = x1
        x1_ = np.sqrt(Vatiation(np.square(y), np.square(x1), args.de, args.la))
    torch.cuda.synchronize()
    elapsed_time = time.time() - start_time
    print(x1_.shape)
    x1 = np.square(x1_)
    #影像输出为tif
    imageio.imsave(os.path.join(args.result_dir,  name[name.__len__()-1] + '_SAR_PNP.tif'),x1)
